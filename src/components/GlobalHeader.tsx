...
export default function GlobalHeader() {
  ...
  // Navigation Links (now includes GAMES)
  const topLinks = [
    ...
    <header className="sticky top-0 z-40 w-full shadow-md font-sans">
      ...
      <div className="bg-[#0e7490] text-white text-sm font-bold uppercase tracking-wide">
        <div className="container mx-auto px-4 h-12 flex items-center justify-between">
          ...
          {/* LEFT: Profile Icon (aligned with chatbot) */}
          <div className="flex items-center gap-4">
            <Link href="/profile/me" className="w-10 h-10 rounded-full bg-zinc-800 border border-white/10 flex items-center justify-center hover:bg-zinc-700 transition-colors shadow-sm">
          ...
          {/* CENTER: Desktop Nav Items (includes GAMES dropdown) */}
          <div className="hidden md:flex items-center gap-6">
            ...
            {/* GAMES Dropdown - same level as APPS/MERCH */}
            <div className="relative">
              ...
              {gamesOpen && (
                <div className="absolute left-0 top-full mt-1 bg-zinc-900 border border-zinc-700 rounded-lg shadow-xl z-50 min-w-[200px]">
                  <Link
          ...
          {/* RIGHT: Hamburger Button (aligned with mini-slots) */}
          <div className="flex items-center gap-4 relative">
            ...
            {/* Hamburger Menu Dropdown - positioned right under button */}
            {hamburgerOpen && (
              <div className="absolute top-full right-0 mt-2 w-80 bg-zinc-900 text-white p-4 flex flex-col gap-4 shadow-2xl z-50 border border-zinc-700 rounded-xl font-sans">

                {/* Mobile Nav Links */}
                <div className="md:hidden space-y-1 pb-4 border-b border-zinc-800">
                  {topLinks.map((link) => (
                    <Link
                      key={link.name}
                      href={link.href}
                      onClick={() => setHamburgerOpen(false)}
                      className="block px-4 py-2 text-sm font-bold hover:bg-zinc-800 rounded-lg transition-colors"
                    >
                      {link.name}
                    </Link>
                  ))}
                  {/* Mobile Games Links */}
                  <Link href="/arcade/play/meme" onClick={() => setHamburgerOpen(false)} className="block px-4 py-2 text-sm font-bold hover:bg-zinc-800 rounded-lg transition-colors">🎭 Name That Meme</Link>
                  <Link href="/arcade/play/pornstar" onClick={() => setHamburgerOpen(false)} className="block px-4 py-2 text-sm font-bold hover:bg-zinc-800 rounded-lg transition-colors">⭐ Who's the Pornstar</Link>
                </div>
